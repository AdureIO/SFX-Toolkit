...
- [x] inline edit is still not working. should work for all types of input -> root cause: the save shelled out `sf data update record --values "Field=Value"`, which breaks on macOS (the literal quotes get shell-escaped into the field name) and on any multi-word value (sf splits on spaces). Rewrote the save to a direct REST PATCH (/sobjects/{Type}/{Id}) via sfRequest with a typed JSON body — works for text/number/date/picklist/reference, and "true"/"false" coerce to boolean, empty → null (clears the field). Also: editable detection now falls back to editable when describe info is missing (so cells aren't silently locked), and the save sends the selected target org.
