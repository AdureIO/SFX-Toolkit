		...
		for (const folderRel of typePaths) {
			...
			if (dirName === "classes" || dirName === "triggers") {
				...
				for (const ent of files) {
					...
					if (ent.isDirectory()) {
						items.push({ label: `${typeLabel}: ${ent.name}`, path: rel });
					} else if (ent.isFile() && (ent.name.endsWith("-meta.xml") || ent.name.endsWith(".xml") || ent.name.endsWith(".permissionset-meta.xml"))) {
						items.push({ label: `${typeLabel}: ${ent.name}`, path: rel });
					}
				}
			}
	...
	if (startChoice.preset) {
		...
		if (missing.length > 0) {
			vscode.window.showErrorMessage(
				`Preset "${preset.name}": some paths no longer exist: ${missing.join(", ")}`
			);
			return;
		}
	...
	if (chosenType.label === DEPLOY_TYPE_RUN_ALL) {
		...
		if (testClassItems.length === 0) {
			vscode.window.showWarningMessage(
				'No test classes found (names containing "Test" or @isTest). Deploy cancelled.'
			);
			return;
		}
