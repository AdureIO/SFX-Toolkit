...
export async function deleteOrg(item: OrgItem) {
  ...
  const cmd = isScratch
    ? `sf org delete scratch -o ${item.orgData.username} -p`
    : `sf org logout -o ${item.orgData.username} -p`;
    ...
    async (_progress, token) => {
      try {
        const res = await runCommand(
          `sf org generate password -o ${item.orgData.username} --json`,
          undefined,
          undefined,
          true,
          token
...
export async function renameAlias(item: OrgItem) {
  ...
  // Existing alias
  let oldAlias = item.orgData.alias || "";
  if (!oldAlias && typeof item.label === "string") {
    oldAlias = item.label;
  }
  oldAlias = oldAlias.replace(/^\s*●\s*/, "");

  let newAlias = await vscode.window.showInputBox({
    prompt: `Enter new alias for ${item.orgData.username}`,
    value: oldAlias
  });

  if (!newAlias) return;
  newAlias = newAlias.replace(/^\s*●\s*/, "");
    ...
    async (_progress, token) => {
      try {
        if (oldAlias && oldAlias !== newAlias) {
          await runCommand(`sf alias unset ${oldAlias}`, undefined, undefined, true, token);
        }
  ...
  try {
    ...
    await vscode.window.withProgress(
      ...
      async (_progress, token) => {
        try {
          await runCommand(
            `sf org create scratch -f "${defFile}" -a ${alias} --duration-days 30 --set-default`,
            undefined,
            undefined,
            true,
            token
