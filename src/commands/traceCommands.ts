      ...
      try {
        ...
        if (!targetDebugLevelId) {
          ...
          if (dlJson.result.records && dlJson.result.records.length > 0) {
            ...
            const anyDlRes = await runCommand(
              `sf data query -q "SELECT Id FROM DebugLevel LIMIT 1" -t --json`,
              undefined,
              undefined,
              true,
              token
        ...
        try {
          ...
          const existingRes = await runCommand(
            `sf data query -q "${existingQuery}" -t --json`,
            undefined,
            undefined,
            true,
            token
