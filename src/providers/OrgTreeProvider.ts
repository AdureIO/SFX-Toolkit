...
import { setOrgListCacheFromSfResult } from "../utils/orgListCache";
...
export class OrgItem extends vscode.TreeItem {
  constructor(
    ...
    if (type === "ORG") {
      ...
      if (isDevHub) {
        this.iconPath = orgData.isDefaultDevHubUsername
...
export class OrgTreeProvider implements vscode.TreeDataProvider<OrgItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<OrgItem | undefined | null | void> = new vscode.EventEmitter<
    OrgItem | undefined | null | void
  >();
  ...
  async getChildren(element?: OrgItem): Promise<OrgItem[]> {
    if (!element) {
      if (!isSalesforceProject()) {
        return [
          new OrgItem("Open an SFDX project", vscode.TreeItemCollapsibleState.None, null, "GROUP", "placeholder")
        ];
      }
      if (!this.data) {
      ...
      if (this.data?._error) {
        if (this.data._error === "sf-not-found") {
          return [
            new OrgItem("Salesforce CLI not found", vscode.TreeItemCollapsibleState.None, null, "GROUP", "placeholder")
          ];
        }
        if (this.data._error.includes("No default") || this.data._error.includes("authenticate")) {
          return [
            new OrgItem("No default org set", vscode.TreeItemCollapsibleState.None, null, "GROUP", "placeholder")
          ];
        }
      }
    ...
    if (element.type === "GROUP") {
      ...
      if (element.contextValue === "group-devhub") {
        ...
        orgs.sort((a: any, b: any) => {
          if (a.isDefaultDevHubUsername && !b.isDefaultDevHubUsername) return -1;
          if (!a.isDefaultDevHubUsername && b.isDefaultDevHubUsername) return 1;
        ...
        return orgs.map(
          (org: any) =>
            new OrgItem(
              org.alias || org.username.split("@")[0],
  ...
  private async fetchOrgs() {
    try {
      ...
      if (json.status === 0) {
        ...
        setOrgListCacheFromSfResult(this.data);
      } else {
