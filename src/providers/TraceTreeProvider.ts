...
export class TraceTreeProvider implements vscode.TreeDataProvider<TraceItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<TraceItem | undefined | null | void> = new vscode.EventEmitter<
    TraceItem | undefined | null | void
  >();
  ...
  async getChildren(element?: TraceItem): Promise<TraceItem[]> {
    ...
    try {
      ...
      const traceItems = traces.map((trace: any) => {
        ...
        const item = new TraceItem(label, description, trace.Id, vscode.TreeItemCollapsibleState.None, {
